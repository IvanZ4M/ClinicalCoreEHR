              ...
              return (
                ...
                >
                  <span style={{
                    ...
                    borderRadius: 'var(--radius-full)', fontSize: '0.75rem', fontWeight: 500, marginBottom: 2,
                ...
                return (
                  <div key={cita.id} style={{ borderLeft: `3px solid ${col.border}`, borderRadius: `0 var(--radius-md) var(--radius-md) 0`, padding: '0.625rem', background: col.bg }}>
                    ...
                    {cita.estado !== 'completada' && cita.estado !== 'cancelada' && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginTop: '0.5rem', paddingTop: '0.5rem', borderTop: '1px solid var(--border)' }}>
                        {cita.estado === 'programada' && (
                          <button onClick={() => cambiarEstado(cita.id, 'confirmada')} style={{ fontSize: '0.6875rem', fontWeight: 500, color: 'var(--ok)', background: 'none', border: 'none', cursor: 'pointer' }}>
                            Confirmar
                          </button>
                        )}
                        {(cita.estado === 'confirmada' || cita.estado === 'programada') && (
                          <button onClick={() => cambiarEstado(cita.id, 'en_sala')} style={{ fontSize: '0.6875rem', fontWeight: 500, color: 'var(--warn)', background: 'none', border: 'none', cursor: 'pointer' }}>
                            Llegó a sala
                          </button>
                        )}
                        {cita.estado === 'en_sala' && (
                          <button onClick={() => { cambiarEstado(cita.id, 'en_consulta'); navigate(`/consulta/nueva?paciente=${cita.paciente}`) }}
                            style={{ fontSize: '0.6875rem', fontWeight: 600, color: 'var(--violet)', background: 'none', border: 'none', cursor: 'pointer' }}>
                            Iniciar consulta
                          </button>
                        )}
                        <button onClick={() => cambiarEstado(cita.id, 'cancelada')} style={{ fontSize: '0.6875rem', color: 'var(--danger)', background: 'none', border: 'none', cursor: 'pointer', marginLeft: 'auto' }}>
                          Cancelar
                        </button>
                      </div>
                    )}
        ...
        <div className="card" style={{ padding: '1rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 2, textAlign: 'center' }}>
            ...
            {obtenerDiasDelMes(mesActual === 11 ? anioActual + 1 : anioActual, mesActual === 11 ? 0 : mesActual + 1)
              .map((dia, idx) => (
                <div key={idx} style={{ fontSize: '0.6875rem', padding: '2px 0', borderRadius: 3, color: dia ? 'var(--text-2)' : 'transparent', cursor: dia ? 'pointer' : 'default' }}>
                  {dia || ''}
                </div>
              ))}
          </div>
        ...
        <div className="modal-overlay" style={{ position: 'fixed', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: '1rem' }}>
          <div className="card anim-scale-in" style={{ width: '100%', maxWidth: 500 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1.25rem 1.5rem', borderBottom: '1px solid var(--border)' }}>
              ...
              <button onClick={() => { setModalAbierto(false); setErrorForm('') }} className="btn btn-ghost btn-icon">
                <I.X width={16} height={16} />
              </button>
            </div>

            <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <MSelect label="Paciente *" value={form.paciente} onChange={v => setForm({ ...form, paciente: v })}>
                <option value="">Seleccionar paciente...</option>
                {pacientes.map(p => <option key={p.id} value={p.id}>{p.nombre} {p.apellidos}</option>)}
              </MSelect>

              <MSelect label="Médico asignado" value={form.medico} onChange={v => setForm({ ...form, medico: v })}>
                <option value="">Sin asignar</option>
                {medicos.map(m => <option key={m.id} value={m.id}>Dr. {m.nombre} {m.apellidos}</option>)}
              </MSelect>

              <MInput label="Fecha y hora *" type="datetime-local" value={form.fecha_hora} onChange={v => setForm({ ...form, fecha_hora: v })} />

              <MSelect label="Tipo de consulta" value={form.tipo} onChange={v => setForm({ ...form, tipo: v })}>
                {TIPOS_CITA.map(t => <option key={t.valor} value={t.valor}>{t.etiqueta}</option>)}
              </MSelect>

              <MInput label="Consultorio" value={form.consultorio} onChange={v => setForm({ ...form, consultorio: v })} placeholder="Ej: Consultorio 1, Sala A..." />

              <div>
                <label className="field-label">Notas adicionales</label>
                <textarea value={form.notas} onChange={e => setForm({ ...form, notas: e.target.value })}
                  rows={2} placeholder="Indicaciones previas, motivo de la cita..." className="input" style={{ resize: 'none' }} />
              </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem', padding: '1rem 1.5rem', borderTop: '1px solid var(--border)' }}>
              <button onClick={() => { setModalAbierto(false); setErrorForm('') }} className="btn btn-outline" style={{ fontSize: '0.875rem' }}>Cancelar</button>
