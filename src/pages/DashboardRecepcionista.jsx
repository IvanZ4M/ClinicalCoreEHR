    ...
    <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }} className="anim-fade">
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.625rem' }}>
          <button onClick={() => navigate('/citas')} className="btn btn-outline" style={{ fontSize: 'var(--fs-2)' }}>
            <I.Calendar width={14} height={14} /> Gestionar citas
          </button>
          <button onClick={() => navigate('/pacientes')} className="btn btn-primary" style={{ fontSize: 'var(--fs-2)' }}>
                  ...
                  <tr key={c.id} className="row-hover" style={{ borderBottom: '1px solid var(--border)' }}>
                    <td style={{ padding: '0.875rem 1.25rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                        <div className="avatar" style={{ width: 32, height: 32, fontSize: 'var(--fs-1)' }}>{initials}</div>
                    ...
                    <td style={{ padding: '0.875rem 1.25rem' }}>
                      <button
                        ...
                        style={{ fontSize: 'var(--fs-1)', padding: '0.375rem 0.75rem', gap: 4 }}
                      >
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <h2 style={{ fontWeight: 600, fontSize: 'var(--fs-3)', color: 'var(--text)' }}>Agenda completa del día</h2>
        </div>
          ...
          <table style={{ width: '100%', fontSize: 'var(--fs-2)', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {citasHoy.map(c => {
                ...
                return (
                  <tr key={c.id} className="row-hover" style={{ borderBottom: '1px solid var(--border)' }}>
                    <td style={{ padding: '0.75rem 1.25rem', fontFamily: 'var(--font-mono, monospace)', fontSize: 'var(--fs-1)', color: 'var(--text-2)', whiteSpace: 'nowrap' }}>
                      {formatearHora(c.fecha_hora)}
                    </td>
...
function VacioFila({ texto, icono }) {
  return (
    <div style={{ padding: '2.5rem', textAlign: 'center', color: 'var(--text-3)', fontSize: 'var(--fs-2)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem' }}>
